<script lang="ts">
  ...
  let fecha1 = fechaActual.toISOString().split("T")[0];
  let fecha2 = fechaActual.toISOString().split("T")[0];
...
<div class="container-lg">
  <form class="">
    ...
    <div class="row">
      <div class="col-md-6">
        <div class="mb-3">
          ...
          <select bind:value={docente} class="form-select" id="selectElement" name="selectElement" on:change={()=>{
            if (docente!=="") td=false;
            consultar();
          }}>
            <option value="" />
      ...
      <div class="col-md-6">
        <div class="mb-3 mt-4 pt-2">
          <button type="submit" class="btn btn-primary w-100" on:click|preventDefault={()=>{
            dispatch('consultar');
          }}>Recargar</button>
        </div>
      </div>
    ...
    <div class="form-check">
        <input bind:checked={td} on:change={()=>{
            if (td) docente="";
            consultar();
